...
/// A failed result carries the reason as text: the HTTP layer returns it as the 400 body, and the bot
/// and training callers treat it as an invariant violation, since their choices are pre-filtered.
/// </summary>
...
/// stopped on Investor by a Swiss Bank (in which case nothing moved and the turn is paused until the
/// responders answer), and what the move cost, for the reward shaping in training.
/// </summary>
public sealed record RondelMoveResult(
    bool Ok,
    string? Error = null,
    bool SwissBankForcedStop = false,
    int? PreviousSlot = null,
    int Cost = 0) : EngineResult(Ok, Error)
{
    public static RondelMoveResult ForcedStop() => new(true, SwissBankForcedStop: true);
