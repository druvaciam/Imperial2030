...
### M7 — Missing rule: a nation's last unoccupied factory province cannot be entered by hostile armies
...
A repo-wide search for any implementation of this protection returns nothing. `ToggleHostility` ([ManeuverController.cs:637](../Server/Controllers/ManeuverController.cs#L637)) flips `IsHostile` with no such check, and `MoveArmy`/`MoveFleet` set `unit.IsHostile = request.IsHostile` freely.

Consequence: a nation reduced to one working factory can be permanently blockaded — no production, no taxation on it, no factory building, no rail — which the rules explicitly prevent. This is a meaningful strategic distortion, and plausibly one the RL bot has learned to exploit.
...
**Action:** add a shared helper — `NationHasSingleUnoccupiedFactory(game, nation)` — and consult it in both places: force `IsHostile = false` on entry into such a province (in `MoveArmy` and in `ToggleHostility`), and scope `DestroyFactory`'s count to unoccupied factories. Per `.agents/AGENTS.md` rule #2, write failing tests first; per rule #24, this qualifies as a provable rulebook contradiction, not a convenience change.
...
### M9 — No optimistic concurrency control on `Game`
...
**Action:** add a `[Timestamp] byte[] RowVersion` to `Game` (migrations for both providers), and handle `DbUpdateConcurrencyException` by reloading and retrying or returning `409`. Lower-cost interim: extend the `_activeBotGames`-style gate into a per-game async lock that HTTP handlers also take.
...
### M11 — The real authentication pipeline has zero test coverage — ✅ MOSTLY ADDRESSED

> **Largely resolved** alongside §H1. `Tests/RealAuthWebApplicationFactory.cs` swaps only the database
> and leaves production authentication intact, supplying `Jwt:Key` through configuration the way a real
> deployment does. `Tests/AuthenticationTests.cs` covers guest issuance/acceptance, guest authorization,
> register → login → authorized call, missing token, and a token signed with the wrong key. **Still
> open:** token-expiry coverage, and the startup key validation described in §H2 (which does not exist
> yet).
...
| L2 | `GetGames` is `[AllowAnonymous]` and returns `UserIds` — raw ASP.NET Identity user GUIDs — for every game to every anonymous caller. Not directly exploitable, but unnecessary exposure; the DTO already carries display names. | [GamesController.cs:68](../Server/Controllers/GamesController.cs#L68) |
| L3 | `GenerateJoinCode` uses `new Random()` per call rather than `RandomNumberGenerator`. Join codes gate private games, so they are a security boundary; 36⁶ is brute-forceable without rate limiting (§H3). | [GamesController.cs:1004](../Server/Controllers/GamesController.cs#L1004) |
| L4 | `GetRankedPlayers` uses `List.Sort` with a comparison returning `0` for absolute ties. `List.Sort` is unstable, so a true tie yields a nondeterministic winner — bad for replay reproduction. Use `OrderBy`/`ThenBy` (stable). | [Game.cs:164–178](../Server/Models/Game.cs#L164) |
| L5 | `GetAvailableBotTypes` performs `Directory.GetFiles` on every call, including from the `[AllowAnonymous]` `available-bots` endpoint and every `AddBot`. Cache at startup. | [GamesController.cs:821](../Server/Controllers/GamesController.cs#L821) |
...
| L7 | Duplicated dead statement: `if (game == null) return NotFound();` appears twice in a row. | [GamesController.cs:1281–1282](../Server/Controllers/GamesController.cs#L1281) |
| L8 | `ToggleHostility` accepts any unit type. Per the rules only armies are placed "standing upright"/"laid on their sides" (p.10); toggling a fleet is meaningless. It also permits toggling in one's own or neutral territory. | [ManeuverController.cs:665](../Server/Controllers/ManeuverController.cs#L665) |
...
| L10 | `Game.PendingInvestorIds` deserializes JSON on every property read and returns a **new** list each time, so `game.PendingInvestorIds.Add(x)` silently does nothing. Current call sites correctly reassign, but the shape invites the bug. Consider a method pair or a cached backing field. | [Game.cs:42–47](../Server/Models/Game.cs#L42) |
...
| L12 | `TriggerBotTurn` is fire-and-forget `Task.Run` with no continuation; `TryPlayBotTurnAsync` rethrows after logging, producing unobserved task exceptions. | [BotService.cs:74](../Server/Services/BotService.cs#L74) |
